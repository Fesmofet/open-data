---
title: Create place object
description: Agent playbook for ODL place object_create and updates.
type: playbook
status: active
scope: platform
tags: [object-create, object-create-playbook, place, agent]
related:
...
Place or location with address and geo.

## When to use / not

- **Use** when creating a new `place` on chain via `odl_build_object_create`.
...
Call `get_object_create_playbook({ object_type: "place" })` for `required_updates` (product policy). Common baseline: `name`, `description`, `image` when supported — not a chain requirement.
