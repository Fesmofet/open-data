...
Hashtag or topic tag for categorization.

## When to use / not

- **Use** when creating a new `hashtag` on chain via `odl_build_object_create`.
...
Call `get_object_create_playbook({ object_type: "hashtag" })` for `required_updates` (product policy). Common baseline: `name`, `description`, `image` when supported — not a chain requirement.
