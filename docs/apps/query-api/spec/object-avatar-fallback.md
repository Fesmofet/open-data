...
description: "When an object has no winning `image` update (avatar), query-api fills `fields.image` from the parent object's image before returning the projection."
type: spec
status: active
scope: query-api
