...
**Back:** [Spec index](../../../spec/README.md) · **Related:** [storage](../../../spec/storage.md), [overflow-strategy](../../../spec/overflow-strategy.md), [architecture](../../../architecture/overview.md)
...
`GET /files/{cid}` returns octet-stream. `GET /content/image/{cid}` returns the same bytes with `Content-Type: image/webp` and inline disposition for browsers and CDNs.
