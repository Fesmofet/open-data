...
| Linked objects | Below advanced settings: search (`/api/search` via `fetchObjectSearchResults`), **Create new object** → `/object-create?return=…` (flush autosave first; `return` is a validated `/editor?draftId=…` path). After object publish, user returns to `/editor?…&attachObject={object_id}`; editor resolves the object via `fetchSearchObjectById`, appends to linked rows (equal split on add), autosaves, strips `attachObject` from the URL. Cards: toggle (detach) and independent percent sliders (equal split only on add/remove). State in draft **`jsonMetadata.objects`**. See [post-json-metadata-objects.md](../../../spec/data-model/post-json-metadata-objects.md), domain [`post-editor-object-create-return.ts`](../../../../apps/web/src/modules/editor/domain/post-editor-object-create-return.ts). Autosave: [editor-drafts.md](editor-drafts.md). |
