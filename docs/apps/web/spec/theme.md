...
**`light`** — default feed UI: page `#fafbfc`, cards `#ffffff`, text `rgba(0,0,0,0.84)` / secondary `#99aab5`, borders `#e9e7e7`, accent orange `#f87007` (`--color-accent`, `--color-link`, `--color-focus`) with alt `#fa9239` and hover `#d96205`, error `#f44336`. Medium-inspired typography: GT Super / Noe Display (`--font-display`), Sohne / Inter (`--font-body`), Charter (`--font-editorial`); scale 13px caption → 16px body → 20px lead → 22px section → 32px display (weights 400 on display/body). Line heights ~1.27 / 1.43 / 1.54 / 1.58. Flat elevation: ring shadows only (`0 0 0 1px` border), 24px stadium buttons (`--radius-btn`) / 8px cards, slightly tighter section spacing.

**`dark`** — dark derivative of **`light`**: same `#f87007` accent family, same Medium-style font stacks and type scale, flat ring shadows on `#191e23` canvas with `#24292e` / `#353535` surfaces and `#4f545c` borders. Used by **`system`** when the OS prefers dark.
