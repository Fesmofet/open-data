...
Staging and production use a **split** Docker Compose layout: an **`infra`** project (Postgres, Redis, IPFS, nginx, Portainer CE) and an **`apps`** project (GHCR images). The **`stack-watchdog`** container periodically **`compose pull`**s (including **migrator**) and **`compose up -d`**s long-running app services so digest updates deploy automatically; operators may still run compose manually or use Portainer. Portainer UI is **`https://127.0.0.1:9443`** on the host only (not via public nginx). Runbooks: [Portainer deployment](../deployment/portainer.md), [stack-watchdog](../apps/stack-watchdog/spec/overview.md).
