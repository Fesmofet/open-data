...
import { fetchUserAuthorityGrantees } from '../../infrastructure/clients/user-permissions.client';
...
export async function getUserAuthorityGranteesPageQuery(
  accountName: string,
  query: UserPermissionsListQuery,
): Promise<PaginatedUserPermissionsList> {
  const page = await fetchUserAuthorityGrantees(accountName, {
    type: query.type,
    sort: query.sort,
    skip: query.skip,
