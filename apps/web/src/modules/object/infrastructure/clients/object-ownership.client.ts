...
export async function fetchObjectOwnership(
  objectId: string,
  args: {
  ...
  const path = `/query/v1/objects/${encodeURIComponent(objectId)}/ownership${qs}`;
  ...
  return queryApiFetch<PaginatedUserFollowListView>(path, {
    headers,
    cacheTags: [queryApiCacheTags.objectOwnership(objectId)],
  });
}
