...
import { WaivWalletHistoryRow } from '../../waiv/history/waiv-wallet-history-row';

function mergeUniqueRows(
  prev: EngineWalletHistoryRowView[],
  more: EngineWalletHistoryRowView[],
): EngineWalletHistoryRowView[] {
...
type EngineWalletHistoryFeedProps = {
  accountName: string;
  page: EngineWalletHistoryPageView;
  loading?: boolean;
  loadError?: EngineWalletHistoryLoadError | null;
  loadMoreAction: (
    accountName: string,
    cursor: string,
...
export function EngineWalletHistoryFeed({
  accountName,
  page,
  loading = false,
  loadError = null,
  loadMoreAction,
}: EngineWalletHistoryFeedProps) {
  const { t } = useI18n();
  const {
    items,
    setItems,
  ...
  const handleLoadMore = useCallback(() => {
    ...
    void loadMoreAction(accountName, cursor)
      .then((next) => {
        ...
        const emptyPage = next.page.items.length === 0;
        setItems((prev) => mergeUniqueRows(prev, next.page.items));
        if (emptyPage) {
          setHasMore(false);
        } else {
    ...
    return (
      <section className="mt-4 rounded-card border border-border bg-surface/80 p-card-padding">
        <p className="text-body-sm text-muted">
          {hasMore || loadingMore ? t('activity_loading') : t('empty_transaction_list')}
        </p>
      </section>
    );
  }
...
export async function loadEngineWalletHistoryPage(
  accountName: string,
  cursor: string,
): Promise<EngineWalletHistoryPageQueryResult> {
  const result = await fetchEngineWalletHistoryPageClient(accountName, {
    cursor,
