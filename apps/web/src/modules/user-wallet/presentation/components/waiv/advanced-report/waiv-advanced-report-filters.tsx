...
import { HiveAdvancedReportAccountsField } from '../../hive/advanced-report/hive-advanced-report-accounts-field';
...
export type WaivAdvancedReportFiltersState = {
  startDate: string;
  endDate: string;
  filterAccounts: string[];
  currency: SupportedCurrency;
  /** When true, swaps and market trades are excluded (default). */
  excludeSwapsAndTrades: boolean;
};

type WaivAdvancedReportFiltersProps = {
  value: WaivAdvancedReportFiltersState;
  onChange: (next: WaivAdvancedReportFiltersState) => void;
  onSubmit: () => void;
...
export function WaivAdvancedReportFilters({
  value,
  onChange,
  onSubmit,
  ...
  dateRangeError = null,
  maxTillDate,
}: WaivAdvancedReportFiltersProps) {
  const { t } = useI18n();
    ...
    >
      ...
      <label className="flex items-center gap-2 text-body-sm">
        <input
          ...
          checked={value.excludeSwapsAndTrades}
          disabled={submitting}
          onChange={(e) =>
            onChange({ ...value, excludeSwapsAndTrades: e.target.checked })
          }
        />
        <span>{t('exclude_swaps_and_trades')}</span>
      </label>
