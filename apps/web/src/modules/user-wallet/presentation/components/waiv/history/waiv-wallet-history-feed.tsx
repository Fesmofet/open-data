...
function mergeUniqueRows(
  prev: WaivWalletHistoryRowView[],
  more: WaivWalletHistoryRowView[],
): WaivWalletHistoryRowView[] {
...
type WaivWalletHistoryFeedProps = {
  accountName: string;
  page: WaivWalletHistoryPageView;
  loading?: boolean;
  loadError?: WaivWalletHistoryLoadError | null;
  showRewards: boolean;
  loadMoreAction: (
    accountName: string,
    cursor: string,
...
export function WaivWalletHistoryFeed({
  accountName,
  page,
  loading = false,
  loadError = null,
  ...
  const handleLoadMore = useCallback(() => {
    ...
    void loadMoreAction(accountName, cursor, showRewards)
      .then((next) => {
        if (next.error) {
          setHasMore(false);
...
export async function loadWaivWalletHistoryPage(
  accountName: string,
  cursor: string,
  showRewards: boolean,
): Promise<WaivWalletHistoryPageQueryResult> {
  const result = await fetchWaivWalletHistoryPageClient(accountName, {
    cursor,
    showRewards,
