...
import type { HomeFeedBody } from './schemas/home-feed.schema';

@Injectable()
export class GetHomeFeedEndpoint {
  constructor(
  ...
  async execute(
    ...
    const feedRows = await this.postsRepo.findHomeFeed(
      viewerTrimmed.length > 0 ? viewerTrimmed : undefined,
      mutedAuthors,
      cursorPayload,
      limitPlusOne,
    );
