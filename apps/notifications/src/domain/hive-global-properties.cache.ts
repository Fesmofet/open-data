...
export type HiveChainContextFields = {
  totalVestingShares: string;
  totalVestingFundSteem: string;
};

const DEFAULT_CHAIN_CONTEXT: HiveChainContextFields = {
...
export class HiveGlobalPropertiesCache {
  ...
  async getChainContextFields(): Promise<HiveChainContextFields> {
    const key = redisKey.hiveGlobalProperties();
    const cached = await this.readRedisJson<HiveChainContextFields>(key);
    if (cached != null) {
  ...
  private async fetchFromHive(): Promise<HiveChainContextFields> {
  ...
  private async writeRedisJson(
    ...
    try {
      ...
      this.logger.warn(
        `hive global properties: failed to write cache for ${key}: ${(e as Error).message}`,
      );
    }
  }
}
